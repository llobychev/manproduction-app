...
function pathHomeMarkup(){
  if(pathState.status==='loading')return renderContentState('loading',{title:'Собираем твой Путь'});
  if(pathState.status==='error')return renderContentState('error',{title:'Путь не загрузился',message:'Твои данные не заменяются демонстрационными значениями.',actionLabel:'Повторить'});
  return `<section class="path-hero"><span class="eyebrow">ТВОЙ ПУТЬ</span><h2>Где ты → куда идёшь</h2><p>Здесь остаётся только главный контур: текущая точка, цель, следующие действия и прогресс. Старые сферы, главы, XP и учебная механика сохранены внутри системы, но не управляют экраном V0.1.</p><div class="path-flow"><span>Сейчас</span><b>→</b><span>Цель</span><b>→</b><span>Действия</span><b>→</b><span>Прогресс</span></div></section><section class="foundation-note"><strong>Путь собирается вокруг твоей жизни</strong><p>Начни с Лёвы: опиши, что происходит. Дальше Путь будет связывать ситуацию с целью и конкретным следующим шагом.</p><button class="primary-button full-width" type="button" data-navigate="lyova.chat">Рассказать Лёве</button></section>`;
}
...
function widgetsHomeMarkup(){
  ...
  const visible=widgetEditor.visible().filter(item=>['finance','tasks','calendar'].includes(item.widgetId));
  return `<section class="widgets-hero"><span class="eyebrow">МОЙ ЭКРАН</span><h2>Виджеты</h2><p>Три базовых инструмента V0.1: финансы, дела и календарь. Остальные модули сохранены для следующих версий, но сейчас не отвлекают от основного контура.</p></section>${visible.length?`<section class="widget-stack">${visible.map(item=>widgetCard(item)).join('')}</section>`:renderContentState('empty',{title:'Базовые виджеты пока не загружены'})}`;
}
...
function lyovaMarkup(active){
  const header=`<section class="lyova-hero"><div class="lyova-avatar">Л</div><div><span class="eyebrow">MENCLUB</span><h2>Лёва</h2><p>Что происходит? Расскажи ситуацию своими словами — отсюда начинается основной контур V0.1.</p></div></section>`;
