...
outlet.addEventListener('click', event => {
  ...
  const legacyOpen=event.target.closest('[data-legacy-open]');if(legacyOpen){const target=legacyOpen.dataset.legacyOpen;if(['fin','goals','schedule'].includes(target)){location.href=`../../index.html?mc_open=${encodeURIComponent(target)}`;}return;}
